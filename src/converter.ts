	...
	for (const img of images) {
		...
		try {
			if (src.startsWith("http://") || src.startsWith("https://")) {
				...
				if (!response.ok) {
					console.warn(
						`Warning: Failed to fetch remote image ${src} (status: ${response.status})`,
					);
					continue;
				}
			...
			console.warn(
				`Warning: Error processing image ${src}: ${(err as Error).message}`,
			);
		}
	}
...
export async function inlineStylesAndFontsInHtml(
	html: string,
	baseDir: string,
): Promise<string> {
	const $ = cheerio.load(html);
	...
	for (const link of links) {
		...
		try {
			...
			if (href.startsWith("http://") || href.startsWith("https://")) {
				...
				if (!response.ok) {
					console.warn(
						`Warning: Failed to fetch stylesheet ${href} (status: ${response.status})`,
					);
					continue;
				}
			...
			console.warn(
				`Warning: Error processing stylesheet ${href}: ${(err as Error).message}`,
			);
		}
	}
	...
	for (const style of styles) {
		...
		for (const match of cssContent.matchAll(urlRegex)) {
			...
			try {
				...
				console.warn(
					`Warning: Error fetching CSS resource ${resourceUrl}: ${(err as Error).message}`,
				);
			}
		}
